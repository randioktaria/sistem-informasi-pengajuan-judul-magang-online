...
if(isset($_POST['ganti'])){
	...
	if(mysqli_num_rows($cek) > 0){
		if($pass_new == $pass_new2){
			$ganti = mysqli_query($kon, "update mahasiswa set password = '$pass_new' where nobp = '$_SESSION[nobp]'");
			if($ganti){
				echo "<script>alert('Password Berhasil Diganti');window.location.href='?p=gantipassmhs';</script>";
			}else{
				echo "<script>alert('Password Gagal Diganti');window.location.href='?p=gantipassmhs';</script>";
			}
		}else{
			echo "<script>alert('Konfirmasi Password Berbeda');window.location.href='?p=gantipassmhs';</script>";
		}
	}else{
		echo "<script>alert('Password Lama Salah');window.location.href='?p=gantipassmhs';</script>";
	}
}
?>